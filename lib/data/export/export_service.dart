...
class ExportService {
  ...
  Future<String> export(AppDatabase db) async {
    ...
    final trainingSets = await db.select(db.trainingSets).get();
    ...
    final payload = <String, Object?>{
      ...
      'training_sets': trainingSets
          .map((t) =>
              {'id': t.id, 'name': t.name, 'kind': t.kind, 'installed_at': t.installedAt.toIso8601String()})
          .toList(),
      ...
      ..addFile(
        ArchiveFile.string(
          ...
          'Memcoach 导出包 $timestamp\n'
              '说明：memcoach_data.json 为全量数据（收藏条目/卡片/复习日志/已下载训练集），\n'
